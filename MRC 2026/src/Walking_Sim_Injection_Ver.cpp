// ============================================================================
//  Walking_Sim_Injection_Ver.cpp
//  PRE-LAUNCH WALKING & FLIGHT SIMULATION INJECTION VERSION FOR ESP32
//
//  Injects recorded pre-launch walking IMU data (to test for false launch triggers)
//  followed seamlessly by OpenRocket/RocketSim flight trajectory data.
//
...
void serial_banner() {
    ...
    Serial.printf("[SIM] WALKING + FLIGHT INJECTION MODE: Active (%u points from combined_sim_data.h)\n", COMBINED_SIM_COUNT);
    Serial.printf("[SIM] Phase 1 (Walking): t=0.0s to %.1fs (real walking acc & gyro)\n", WALKING_PHASE_END_TIME_S);
    Serial.printf("[SIM] Phase 2 (Pad/Flight): Launch at %.1fs | End at %.1fs\n",
                  FLIGHT_LAUNCH_TIME_S, COMBINED_SIM_DATA[COMBINED_SIM_COUNT - 1].time_s);
...
void setup() {
    ...
    while (millis() < cal_end) {
        ...
        if (!std::isfinite(p) || p < 1000.0f) {
            p = COMBINED_SIM_DATA[0].pressure_pa; // Use simulation ground pressure baseline
        }
        pres_sum += p;
        pres_cnt++;
...
void loop() {
    ...
    // ── Initial Sensor Reading Phase (Walking + Flight Injected) ──────────────
    float sim_t_s = t_ms * 0.001f;
    CombinedSimDataPoint sample;
    bool  sim_active = combined_sim_get_sample(sim_t_s, sample);

    // 1. Barometer reading injection
    float pres_pa  = sample.pressure_pa;
    float baro_raw = pressure_to_alt(pres_pa);
    float baro_alt = baro_smooth_push(baro_raw);   // causal 3-sample average

    // 2. IMU reading injection
    BMI160Data imu;
    if (sample.is_flight == 0) {
        // --- WALKING / PRE-LAUNCH PHASE: Inject real measured 6-DOF IMU data ---
        imu.ax = sample.ax;
        imu.ay = sample.ay;
        imu.az = sample.az;
        imu.gx = sample.gx;
        ...
        // --- FLIGHT SIMULATION PHASE: Inject trajectory acceleration ---
        float axial_target = sample.vert_accel + G + accel_bias_axial;
#if AXIAL_AXIS == 0
        imu.ax = ((float)AXIAL_SIGN) * axial_target;
        imu.ay = sample.lat_accel;
        imu.az = 0.0f;
#elif AXIAL_AXIS == 1
        imu.ax = sample.lat_accel;
    ...
    if (!sim_completed && !sim_active && (t_ms >= (unsigned long)(COMBINED_SIM_DATA[COMBINED_SIM_COUNT - 1].time_s * 1000.0f))) {
        sim_completed = true;
        Serial.println("\n============================================================");
