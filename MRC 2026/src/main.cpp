// ============================================================================
//  Sim_Injection_NoSD_CO2_1360us.cpp
...
//  CO2 failsafe timer. EEPROM is the only persistent event recorder.
// ============================================================================
...
// CO2 release servo uses direct pulse-width commands rather than degree mapping.
// Safe/initial position follows the tested sequence: 700 us.
// Ejection position requested: 1360 us.
static const int CO2_INITIAL_PULSE_US              = 700;
static const int CO2_EJECTION_PULSE_US             = 1360;
...
// Logical flight identifier used by the EEPROM history.
extern char flight_id[32];
...
uint16_t eeprom_count_unique_flights() {
    ...
    char known_ids[EEPROM_MAX_RECORDS][32];
    ...
    for (uint16_t i = 0; i < eeprom_header.count; i++) {
        ...
        for (uint16_t j = 0; j < unique_count; j++) {
            if (strncmp(known_ids[j], record.flight_id,
                        sizeof(known_ids[j])) == 0) {
                already_found = true;
                break;
            }
        }
        ...
        if (!already_found && unique_count < EEPROM_MAX_RECORDS) {
            snprintf(known_ids[unique_count],
                     sizeof(known_ids[unique_count]),
                     "%s", record.flight_id);
            unique_count++;
        }
    }
...
void eeprom_print_history() {
    ...
    // Build a chronological list of unique flight IDs, then print each
    // flight as its own section. This keeps states belonging to flight_0001,
    // flight_0002, etc. visually separated.
    char flight_ids[EEPROM_MAX_RECORDS][32];
    memset(flight_ids, 0, sizeof(flight_ids));
    ...
    for (uint16_t i = 0; i < eeprom_header.count; i++) {
        ...
        const char *record_id = record.flight_id[0]
                                      ? record.flight_id
                                      : "UNKNOWN";
        bool already_listed = false;
        for (uint16_t j = 0; j < flight_count; j++) {
            if (strncmp(flight_ids[j], record_id,
                        sizeof(flight_ids[j])) == 0) {
                already_listed = true;
                break;
            }
        }
        ...
        if (!already_listed && flight_count < EEPROM_MAX_RECORDS) {
            snprintf(flight_ids[flight_count],
                     sizeof(flight_ids[flight_count]), "%s", record_id);
            flight_count++;
        }
    }
    ...
    for (uint16_t flight_index = 0; flight_index < flight_count; flight_index++) {
        ...
        bool numbered_file =
            (sscanf(flight_ids[flight_index], "FLIGHT_%lu",
                    &stored_flight_number) == 1) ||
            (sscanf(flight_ids[flight_index], "/flight_%lu.csv",
                    &stored_flight_number) == 1);
        ...
        Serial.printf("Flight ID: %s\n", flight_ids[flight_index]);
        ...
        for (uint16_t i = 0; i < eeprom_header.count; i++) {
            ...
            const char *record_id = record.flight_id[0]
                                          ? record.flight_id
                                          : "UNKNOWN";
            if (strncmp(record_id, flight_ids[flight_index],
                        sizeof(record.flight_id)) != 0) {
                continue;
            }
...
bool eeprom_store_flight_event(unsigned long time_ms, float altitude_m,
    ...
    snprintf(record.flight_id, sizeof(record.flight_id), "%s",
             flight_id[0] ? flight_id : "NO_ID");
...
// Logical flight identifier for EEPROM event grouping.
char flight_id[32] = "FLIGHT_0001";
...
// Forward declaration because the servo helpers also store deployment events
// in EEPROM. The implementation remains in the event-log section below.
...
void initialise_deployment_servos() {
    ...
    co2EjectionServo.writeMicroseconds(CO2_INITIAL_PULSE_US);
    ...
    Serial.printf("[SERVO] CO2 EJECTION  GPIO %d -> initial %d us\n",
                  CO2_EJECTION_SERVO_PIN, CO2_INITIAL_PULSE_US);
}

void deploy_co2(unsigned long t_ms, const char *reason) {
    if (co2_ejected) return;

    // writeMicroseconds() continuously commands 1360 us until another command
    // is issued, so a blocking delay(5000) is not required and would interrupt
    // the 100 Hz estimator/deployment loop.
    co2EjectionServo.writeMicroseconds(CO2_EJECTION_PULSE_US);
    co2_ejected = true;

    Serial.println("\n============================================================");
    Serial.printf("[DEPLOY] >>> CO2 EJECTION SERVO OPENED at %.3f s\n", t_ms / 1000.0f);
    Serial.printf("[DEPLOY] >>> GPIO %d | %d us -> %d us\n",
                  CO2_EJECTION_SERVO_PIN, CO2_INITIAL_PULSE_US, CO2_EJECTION_PULSE_US);
...
void terminal_execute_command(const char *command) {
    if (strcmp(command, "clr") == 0) {
        ...
        Serial.printf("Current armed marker    : %s (%s)\n",
                      marker_saved ? "SAVED" : "NOT SAVED", flight_id);
...
void serial_banner() {
    ...
    Serial.println("Red codes: 1=EEPROM, 2=BMI160, 3=BMP280, 4=calibration.");
    ...
    Serial.printf("[SERVO] CanSat BOTTOM GPIO%d: initial=%d deg, open=%d deg\n",
                  CANSAT_BOTTOM_SERVO_PIN, CANSAT_BOTTOM_INITIAL_ANGLE_DEG,
                  CANSAT_BOTTOM_OPEN_ANGLE_DEG);
    Serial.printf("[SERVO] CO2 GPIO%d: initial=%d us, eject=%d us\n",
                  CO2_EJECTION_SERVO_PIN, CO2_INITIAL_PULSE_US, CO2_EJECTION_PULSE_US);
...
void serial_print_armed_summary() {
    ...
    bool numbered_file = sscanf(flight_id, "FLIGHT_%lu",
    ...
    if (numbered_file) {
        ...
        Serial.printf("Current flight ID       : %s\n", flight_id);
    }
...
void store_flight_event(unsigned long time_ms, float altitude_m,
                        FlightPhase state, const char *action) {
    // EEPROM is the sole persistent event recorder.
...
void setup() {
    ...
    // Allocate a logical flight number for EEPROM grouping. This replaces the
    // old CSV-style identifier while preserving readable per-flight history.
    unsigned int next_flight_number = eeprom_ready
                                        ? (unsigned int)eeprom_count_unique_flights() + 1U
                                        : 1U;
    snprintf(flight_id, sizeof(flight_id), "FLIGHT_%04u", next_flight_number);
    Serial.printf("[EEPROM] Current flight ID: %s\n", flight_id);
    ...
    Serial.printf("[EVENT] >>> CO2 servo: GPIO %d ready at %d us\n",
                  CO2_EJECTION_SERVO_PIN, CO2_INITIAL_PULSE_US);
...
void loop() {
    ...
    // ── Initial Sensor Reading Phase (Simulation Injected) ───────────────────
    ...
    if (!sim_completed && !sim_active && (t_ms >= (unsigned long)(SIM_DATA[SIM_DATA_COUNT - 1].time_s * 1000.0f))) {
        ...
        Serial.println("[EVENT] >>> Flight events logged to EEPROM.");
        Serial.println("============================================================\n");
    }
